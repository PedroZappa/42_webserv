...
std::ostream &operator<<(std::ostream &os, const Cluster &cluster) {
    ...
    os << BBLK << Logger::currentTime() << BCYN " INFO " NC "Cluster Info:\n";

    // Print Epoll FD
    os << BYEL "Epoll fd: " NC << cluster.getEpollFd() << "\n";

    // Print all Virtual Servers
    os << BYEL "Virtual Servers:\n" NC;
    const std::vector<Cluster::VirtualServer> &virtualServers =
        cluster.getVirtualServers();
    for (std::size_t i = 0; i < virtualServers.size(); ++i) {
        os << "  [" << i << "] " << virtualServers[i] << "\n";
    }

    // Print Listening Sockets
    os << BYEL "Listening Sockets:\n" NC;
    const std::vector<int> &listeningSockets = cluster.getListeningSockets();
    for (std::size_t i = 0; i < listeningSockets.size(); ++i) {
...
std::ostream &operator<<(std::ostream &os,
                         const Cluster::VirtualServer &server) {
    os << BGRN "VirtualServer " NC "(\n"
       << BMAG "\tName" NC ": " << server.name << ",\n"
       << BMAG "\tIP" NC ": " << server.ip << ",\n"
       << BMAG "\tPort" NC ": " << server.port << "\n  )";
    return (os);
}
...
std::set<Socket> Cluster::getVirtualServerSockets(void) {
    ...
    for (vsIt3 = virtualServers.begin(); vsIt3 != virtualServers.end();
         ++vsIt3) {
...
#ifdef DEBUG
    Logger::debug("Cluster", __func__,
                  "setting up socket: " YEL + ip + ":" + port + NC);
#endif
 ...
 * @throw std::runtime_error if the socket cannot be added to the epoll
 * instance.
 */
void Cluster::setEpollSocket(int socket) {
#ifdef DEBUG
    Logger::debug("Cluster", __func__,
                  "adding socket (epoll_event) to epoll instance");
#endif
 ...
 * @throw std::runtime_error if the socket cannot be removed from the epoll
 * instance.
 */
 ...
 *
 * @return const std::vector<const Server*>& A reference to the vector of
 * servers.
 */
 ...
 *
 * @return const std::vector<int>& A reference to the vector of listening
 * sockets.
 */
 ...
 */
int Cluster::getEpollFd(void) const { return (_epollFd); }
/** @} */
