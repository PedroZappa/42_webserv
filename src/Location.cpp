...
std::vector<std::string> Location::getIndex(void) const { return (_index); }
...
State Location::getAutoIndex(void) const { return (_autoIndex); }

/// @brief Get the CliMaxBodySize value
long Location::getClientMaxBodySize(void) const { return (_clientMaxBodySize); }
...
std::string Location::getUploadStore(void) const { return (_uploadStore); }
...
std::set<Method> Location::getValidMethods() const { return _validMethods; }
...
#ifdef DEBUG
    Logger::debug("Location", __func__,
                  "Processing directive: " YEL + directive + NC);
#endif
...
#ifdef DEBUG
    Logger::debug("Location", __func__,
                  "Set directive: " YEL + directive + " " NC);
#endif
}
...
#ifdef DEBUG
    Logger::debug("Location", __func__,
                  "Processing root directive: " YEL + root + NC);
#endif
...
#ifdef DEBUG
    Logger::debug("Location", __func__,
                  "Processed index directive: " YEL + _index[0] + NC);
#endif
}
...
void Location::setLimitExcept(std::vector<std::string> &tks) {
    ...
    Logger::debug("Location", __func__,
                  "Processed limit_except directive: " YEL);
#endif
}
...
#ifdef DEBUG
    Logger::debug("Location", __func__,
                  "Processed directive: " YEL + tks[0] + " " + tks[1]);
#endif
}
