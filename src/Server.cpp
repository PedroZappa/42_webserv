 ...
 * @param copy The Server object to copy from.
 */
 ...
 * @param copy The Server object to assign from.
...
/**
 * @brief Output stream operator for the Server class.
 * @param os The output stream.
 * @param ctx The Server object to output.
...
bool Server::isIpValid(const std::string &ip) const {
    ...
    while (std::getline(iss, seg, '.')) {
        ...
        for (i = 0; i < seg.size(); i++)
            if (!std::isdigit(
                    seg[i])) // Check if the string contains only digits
                return (false);
...
bool Server::isPortValid(const std::string &port) const {
    ...
    std::size_t portN = std::atoi(port.c_str());
...
/// @return A vector of Socket objects representing the network addresses of the
/// server.
std::vector<Socket> Server::getNetAddr(void) const { return (_netAddr); };
...
long Server::getClientMaxBodySize(void) const { return (_clientMaxBodySize); }
...
/// @brief Returns the upload store.
/// @return The upload store.
std::string Server::getUploadStore(void) const { return (_uploadStore); }
...
/// @brief Returns the cgi extension.
/// @return The cgi extension.
std::string Server::getCgiExt(void) const { return (_cgiExt); }
...
#ifdef DEBUG
    Logger::debug("Server", __func__,
                  "Processed directive: " YEL + _serverName[0] + NC);
#endif
}
...
void Server::setClientMaxBodySize(std::vector<std::string> &tks) {
    ...
    if (!std::isdigit(unit)) {
        ...
        case 'G':
            ...
            break;
        default:
            throw std::runtime_error("Invalid unit for max_body_size: " +
                                     tks[1]);
        }
    }
    ...
    Logger::debug("Server", __func__,
                  "Processed directive: " YEL + tks[0] + NC);
#endif
}
...
#ifdef DEBUG
    Logger::debug("Server", __func__,
                  "Processing root directive: " YEL + root[0] + NC);
#endif
...
#ifdef DEBUG
    Logger::debug("Server", __func__,
                  "Processed root directive: " YEL + _root + NC);
#endif
}
...
void Server::setLocation(std::string block, size_t start, size_t end) {
    ...
    while (std::getline(location, line, ';')) {
        ...
        // if (discard != "root" && discard != "autoindex" && discard !=
        // "index") 	throw std::runtime_error("Invalid location block:
        // unknown directive");
...
#ifdef DEBUG
    Logger::debug("Server", __func__,
                  "Processed directive: " YEL + _uploadStore);
#endif
}
...
#ifdef DEBUG
    Logger::debug("Server", __func__,
                  "Processed directive: " YEL + _return.second);
#endif
}
...
void Server::setIPaddr(const std::string &ip,
                       struct sockaddr_in &sockaadr) const {
    if (ip.empty())
